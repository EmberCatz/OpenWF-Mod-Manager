...
| 3 | Auth token stored in plaintext `localStorage` + Tauri CSP fully disabled (`security.csp: null`) — combined, a single future XSS becomes a full account-takeover primitive with nothing to contain it. | `apps/desktop/src/settings.ts:59-65`, `apps/desktop/src-tauri/tauri.conf.json:22-24` | **Done** |
...
| 2 | Comment vote race condition — rapid double-vote can leave `score`/`myVote` inconsistent with the server; no request sequencing/cancellation. | `apps/desktop/src/components/CommentSection.tsx:210-224` | **Done** |
...
4. **Harden `handleVote` in `CommentSection.tsx`.** *(Done — a per-comment sequence counter (`voteSeqRef`) is bumped on every vote; a response is only applied if its sequence number is still the latest for that comment, otherwise it's discarded as superseded. Verified live by forcing an out-of-order response: patched `fetch` so an upvote resolves 900ms late and a downvote resolves immediately, then fired both in quick succession — the log confirmed the downvote's response really did land first and the upvote's late response arrived after; final state correctly stayed at the downvote (`score: -1, down-active`) instead of being clobbered back to the stale upvote.)*
5. **Move the API key to the OS keychain; set a real `security.csp`.** *(Done — the token now lives in the OS keychain (Windows Credential Manager / macOS Keychain / Linux Secret Service) via the Rust `keyring` crate, behind three named commands (`get_api_key`/`set_api_key`/`clear_api_key`, `apps/desktop/src-tauri/src/commands.rs`) — there's no generic "read everything" API to it the way `localStorage.getItem` was. `getApiKey`/`setApiKey` in `settings.ts` are now async as a result; the two render-body call sites (`Admin.tsx`, `MyMods.tsx`) moved to a new `useApiKey()` hook (`useAccount.ts`) matching `useAccount`'s own fetch-on-mount shape. `tauri.conf.json`'s `security.csp` is a real policy now (`script-src 'self'`, no `unsafe-eval`; `connect-src` covers the Worker API plus the `ipc:`/`http://ipc.localhost` sources Windows' IPC bridge requires; `frame-src` preserves the Live Settings/Server WebUI `localhost` iframes; `style-src`/`font-src` cover the existing Google Fonts usage) — verified against Tauri's own docs for the Windows-specific `ipc:` requirement before writing it, since getting that wrong would have silently broken every `invoke()` call in the app. Verified: `cargo check` compiles the new dependency; a standalone Rust program exercising the exact same `keyring::Entry` API (set → get → delete → get-returns-NoEntry) round-tripped correctly against this machine's real Windows Credential Manager; `tsc --noEmit` and a full `vite build` both pass. Not verified live end-to-end inside the actual packaged Tauri window — no GUI available in this environment — so give login/logout a real try next time you're in the app.)*
